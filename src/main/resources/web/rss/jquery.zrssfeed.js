...
(function (factory) {
	...
	$.fn.rssfeed = function(url, options, fn) {	
		...
		return this.each(function(i, e) {
			...
			//var api = "http"+ s +"://www.feedrapp.info/?v=1.0&callback=?&q=" + encodeURIComponent(url);
			//api += "&num=" + options.limit;
			var api = "http"+ s +"://rss2json.com/api.json?rss_url=" + encodeURIComponent(url);
			api += "&items=" + options.limit;
			// Send request
	...
	var _process = function(e, data, options) {
		...
		// Add feeds
		for (var i=options.offset; i<data.items.length; i++) {
			...
			// Add feed row
			// read enclosed image
			if(entry.enclosure != null && entry.enclosure['type'] == "image/jpeg" ) {
				rowArray[rowIndex]['html'] = "<img class='enclosedImage' src='"+entry.enclosure['link']+"' />";
			}else {
