	...
	function jqCron(settings) {
		...
		this.init = function(){
			...
			for(i=0, list=settings.minutes; i<list.length; i++){
				//CUSTOM _selectorMins.add(list[i], list[i]);
				_selectorMins.add(list[i], ("0" + Number(list[i])).slice(-2));
			}
			...
			for(i=0, list=settings.hours; i<list.length; i++){
				//CUSTOM_selectorTimeH.add(list[i], list[i]);
				_selectorTimeH.add(list[i], ("0" + Number(list[i])).slice(-2));
			}
			_selectorTimeM = newSelector(_$blockTIME, settings.multiple_time_minutes, 'time_minutes');
			for(i=0, list=settings.minutes; i<list.length; i++){
				//CUSTOM_selectorTimeM.add(list[i], list[i]);
				_selectorTimeM.add(list[i], ("0" + Number(list[i])).slice(-2));
			}
